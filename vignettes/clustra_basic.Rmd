...
```
**Notes: Next, finish the rand index refactoring with random starts. Make sure exit_report has what is needed.**
...
```{r fig.width = 7}
sil = clustra_pic_k(data, type = "silhouette", verbose = TRUE)
...
ran = clustra_pic_k(data, type = "rand", verbose = TRUE)
rand_plot(ran)
```