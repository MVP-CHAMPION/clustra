...
run_extra = FALSE
```
The **clustra** package was built to **clus**ter medical **tra**jectories (time series) related by an intervention time. This vignette main purpose is to reproduce the figures in the paper "clustra: A multi-platform k-means clustering algorithm for analysis of longitudinal trajectories in large electronic health records data." The plots are rendered in the vignette and also saved in a temp directory. An exception is the Rand index plot, which takes a long time to produce, especially if using less than 5 cores. To enable its production, set `rand = TRUE` in the chunk above.

Additional plots to explain the methodology and examine the data can be produced, but these code chunks do not evaluate by default so the vignette runtime is shorter. To enable the additional sections, set `extra = TRUE` in the chunk above.
...
# On Intel chips with hyperthreading, up to 2x cores can be used.
...
```{r, plot_true, fig.width = 7, fig.height = 9, eval = run_extra}
set.seed(12345)
...
```{r, plot_raw, fig.width = 7, fig.height = 7, eval = run_extra}
...
set.seed(12345)
cl5.v2 = clustra(data, k = 5, maxdf = 30, conv = c(5, 0.5), mccores = mc, verbose = FALSE)
```
...
```{r, iter_plots, fig.width=7, fig.height=7, eval = run_extra}
set.seed(12345)
clustra(data, k = 5, maxdf = 30, conv = c(10, 0.5), mccores = mc, verbose = 2, ylim = c(110, 170))
```

Test producing two side-by-side plots.
```{r, plot5_side_by_side, fig.width = 7, fig.height = 5, warnings=FALSE, eval = run_extra}
...
```{r fig1, fig.width = 7, fig.height = 5, eval = run_extra}
...
```{r, comp_true}
MixSim::RandIndex(cl5$data_group, data[, true_group])$AR # compare the 20-max-iterations to true assignemnt
MixSim::RandIndex(cl5.v2$data_group, data[, true_group])$AR # compare the 10-max-iterations to true assignment
MixSim::RandIndex(cl5$data_group, cl5.v2$data_group)$AR # compare the 10-max-iterations and the 20-max-iterations assignments
```
...
```{r rand2, comp.within, eval = run_extra}
MixSim::RandIndex(cl5$data_group,cl2$data_group)$AR # between 5 and 2 clusters
MixSim::RandIndex(cl5$data_group,cl10$data_group)$AR # between 5 and 10 clusters
...
t = deltime()
set.seed(12345)
sil = clustra_sil(cl5, mccores = mc, conv=c(20,0.5), verbose = FALSE)
...
```{r silplot2, fig.width = 7, fig.height = 5, eval = run_extra}
...
```{r, rand_plot, fig.width = 6.5, fig.height=6, eval = run_rand}
t = deltime()
set.seed(12345)
...
```
```{r, hclust_run40, fig.width = 7, fig.height = 5, eval = run_extra}
set.seed(12345)
t = deltime()
...
```
The cluster dendrogram indicates there are five clusters. Making the cut at a height of roughly 300 groups the 26 clusters into only three.

## Comparison between SAS and R clustering 
Look at the data output from the SAS clustering analysis. Since SAS and R produce slightly different labels, need to relabel and replot the SAS data output. The relabeling algorithm below matches pairs by distance, starting with shortest distance.

```{r SAS_R_data, fig.width = 7, fig.height = 5}
sas_git = "~/Git/mvp-champion/sas-trajectories-macros/"
sas_graphout = paste0(sas_git, "Manuscript Files/Graph Data/")
sas_rms = paste0(sas_git, "Manuscript Files/")

sas_r_cplot = function(sasfile, clustra_out) {
  sas = cbind(haven::read_sas(sasfile), source = 1)
...
png(paste0(plot_path, "R_SAS_cl2_compare.png"), width = 480, height = 360)
  map2 = sas_r_cplot(paste0(sas_graphout, "graphout_cl2.sas7bdat"), cl2)
dev.off()
png(paste0(plot_path, "R_SAS_cl5_compare.png"), width = 480, height = 360)
  map5 = sas_r_cplot(paste0(sas_graphout, "graphout_cl5.sas7bdat"), cl5)
dev.off()
png(paste0(plot_path, "R_SAS_cl10_compare.png"), width = 480, height = 360)
  map10 = sas_r_cplot(paste0(sas_graphout, "graphout_cl10.sas7bdat"), cl10)
dev.off()
...
```{r Rand_SAS_R}
sas_rms = paste0(sas_git, "Manuscript Files/")
MixSim::RandIndex(haven::read_sas(paste0(sas_rms, "rms_cl2.sas7bdat"))$newgroup, cl2$group)
MixSim::RandIndex(haven::read_sas(paste0(sas_rms, "rms_cl5.sas7bdat"))$newgroup, cl5$group)
MixSim::RandIndex(haven::read_sas(paste0(sas_rms, "rms_cl10.sas7bdat"))$newgroup, cl10$group)
```
...
MixSim::RandIndex(haven::read_sas(paste0(sas_rms, "rms_cl5.sas7bdat"))$newgroup, true_group$V1)
