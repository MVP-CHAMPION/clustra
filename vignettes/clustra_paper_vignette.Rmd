---
title: "clustra: clustering trajectories (graphics for paper)"
...
We start by loading the necessary R packages and grabbing the data that was generated for this example. We also set the output path for plots to `tempdir()`, which should be changed to a user directory if they are needed beyond the Knit output.
...
Look at the data output from the SAS clustering analysis. Since SAS and R produce slightly different labels, need to relabel and replot the SAS data output. The relabeling algorithm below matches pairs by distance, starting with shortest distance pair.

```{r SAS_R_data, fig.width = 7, fig.height = 5}
sas.file = function(file) system.file("extdata", file, package = "clustra")
