...
clus1<-defDataAdd(varname="eGFR",formula="-0.02*time+60",variance=1)
#clus1<-defDataAdd(clus1,varname = "RE", dist = "normal",formula = 0,variance = sqrt(3))
#clus1<-defDataAdd(varname="sys",formula="0.0375*time+85",variance=50)
clus1<-defDataAdd(clus1,varname="V2",formula="0.008*time+28",variance=1)

clus2<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-200)/200*5))))",variance=1)
#clus2<-defDataAdd(clus2,varname = "RE", dist = "normal",formula = 0,variance = sqrt(3))
clus2<-defDataAdd(clus2,varname="V2",formula="20+sin(-50+3.14159*time/300)*3",variance=1)

clus3<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-300)/300*5))))",variance=1)
#clus3<-defDataAdd(clus3,varname = "RE", dist = "normal",formula = 0,variance = sqrt(3))
clus3<-defDataAdd(clus3,varname="V2",formula="-0.02*time+15",variance=1)

### second scenario

clus4<-defDataAdd(varname="eGFR",formula="-0.02*time+60",variance=1)
#clus4<-defDataAdd(clus4,varname="V2",formula="55",variance=1)
#clus4<-defDataAdd(clus4,varname="V2",formula="25+5*sin(-3.1415926*((time-20)/250)-20)",variance=0.5)
clus4<-defDataAdd(clus4,varname="V2",formula="25+20*sin(-3.1415926*((time-20)/250)-20)",variance=0.5)

clus5<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-200)/200*5))))",variance=1)
#clus5<-defDataAdd(clus5,varname="V2",formula="35",variance=1)
clus5<-defDataAdd(clus5,varname="V2",formula="30+5*sin(-3.1415926*((time-30)/300)-34)",variance=0.5)



clus6<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-300)/300*5))))",variance=1)
clus6<-defDataAdd(clus6,varname="V2",formula="0.001*time+15",variance=1)



### Third scenario
clus7<-defDataAdd(varname="eGFR",formula="-0.02*time+60", variance=1)
clus7<-defDataAdd(clus7,varname="V2",formula="70+45*sin(-3.1415926*((time-30)/350)-34)",variance=2)
#clus7<-defDataAdd(clus7,varname="V2",formula="-0.15*time+75",variance=1)
#clus7<-defDataAdd(clus7,varname="V2",formula="25+sin(-3.1415926*(time-20)/250-20)",variance=1)

clus8<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-200)/200*5))))",variance=1)
clus8<-defDataAdd(clus8,varname="V2",formula="-0.0015*time^2+0.96*time+30",variance=2)
#clus8<-defDataAdd(clus8,varname="V2",formula="-0.0024*time^2+0.87*time",variance=1)

clus9<-defDataAdd(varname="eGFR",formula="60*(1-(1/(1+2.71828^(-(time-300)/300*5))))",variance=1)
clus9<-defDataAdd(clus9,varname="V2",formula="50+45*sin(-3.1415926*((time-30)/350)-34)+0.4*time",variance=2)
#clus9<-defDataAdd(clus9,varname="V2",formula="0.15*time+15",variance=1)
...
#curv3=c(14,15,16)
 ...
 curvlist=list(curv1,curv2,curv3)
  ...
  data = rep(list(NULL),length(curvlist))
  for(q in 1:length(curvlist)){
data[[q]] = gen_traj_data(vars, clusters = c(2000, 2000,2000), m_obs = 10, 
                     curvlist=curvlist[[q]], 
                     s_range = c(-1, -1), e_range = c(0.5*365, 365),
 ...
 data[[s]]$eGFR_std <- scale(data[[s]]$eGFR)[, 1]
data[[s]]$V2_std <- scale(data[[s]]$V2)[, 1]
}
vars = c("eGFR_std", "V2_std")
...
library(gridExtra)
 ...
 for(s in 1:length(curvlist)){
psys = ggplot(data[[s]], aes(x = time, y = eGFR, color = as.factor(true_group))) + 
  geom_point(data = data[[s]], aes(y = eGFR), pch = ".") #+ geom_line()


pdys = ggplot(data[[s]], aes(x = time, y = V2, color = as.factor(true_group))) + 
  geom_point(data = data[[s]], aes(y = V2), pch = ".")
