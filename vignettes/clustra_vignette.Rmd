...
cl = clustra(data, k = 3, 
             ...
             maxdf = 30, iter = 10, mccores = mc, verbose = TRUE)
```
...
```{r fig.width = 7}
set.seed(1234737)
sil = clustra_sil(data, k = c(2, 3, 4), 
                  model = list(id = "id", resp = c("response"), time = "time"),
...
```{r fig.width = 7, fig.height=7}
set.seed(1234737)
ran = clustra_rand(data, k = c(2, 3, 4),
                   model = list(id = "id", resp = c("response"), time = "time"),
                   mccores = mc, replicates = 10, verbose = TRUE)
rand_plot(ran)
```
The plot shows Adjusted Rand Index similarity level between all pairs of 30 clusterings (10 random starts for each of 2, 3, and 4 clusters). The ten random starts agree the most for *k*=3. From the deviance results shown during iterations, we also see that all of the *k*=3 clusters are near the best deviance attainable even with *k* = 4. Among the *k* = 4 results, in each replicate, two of four clusters also show the same cluster counts as the *k* = 3 cluster results and split a third cluster into two parts.
