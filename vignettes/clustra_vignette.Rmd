...
```{r}
set.seed(1234737)
cl = clustra(data, k = 3, fp = list(maxdf = 30, iter = 10), verbose = TRUE)
```
...
cl = clustra(data2, k = 3, fp = list(maxdf = 30, iter = 10), verbose = TRUE)
MixSim::RandIndex(cl$data_group, data2[, true_group])
```
Next, we try requesting too many clusters in the noisy data.
```{r}
set.seed(1234737)
cl = clustra(data2, k = 20, fp = list(maxdf = 30, iter = 20), verbose = TRUE)
```
Next, plot the resulting fit with a sample of data, colored by the cluster value.
```{r fig.width = 7, fig.height = 7}
...
rand_plot(ran)
```
The plot shows similarity level between all pairs of 30 clusterings (10 random starts for each of 2, 3, and 4 clusters). The ten random starts agree the most for *k* = 2 and *k*=3, making it difficult to decide. But considering deviance reported earlier during iterations, we see that seven of the *k*=3 clusters are near the best deviance attainable even with *k* = 4. Also, interestingly, two of the four cluster attempts collapse to 3 clusters. 
```{r}
## restore RNG
RNGkind(rng_prev[1])
```
...
To use conda for external dependency management requires additional steps before starting R.

conda setup:
conda activate (for version 4.3 currently in KDI):
* `source anaconda3/bin/conda` activates conda
* `conda info` gives conda version
* `conda info --envs` give installed environments
* `source activate R` will activate the R environment (after anaconda3 above is already activated)
