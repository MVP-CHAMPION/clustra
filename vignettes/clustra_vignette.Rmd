...
sdt = data[[1]]
if(nrow( data[[1]]) > 10000)
  sdt =  data[[1]][, group:=factor(..cl$data_group)][sample(nrow( data[[1]]), 10000)]

for(v in 1:length(vars)){
ggplot(sdt, aes(x = time, y = vars[v])) + geom_point(pch = ".")

np = 100
k = length(cl$myTPSlist)
ntime = seq( data[[1]][, min(time)],  data[[1]][, max(time)], length.out = np)
pdata = expand.grid(time = ntime, group = factor(1:k))
pdata = subset(pdata, group %in% which(lengths(cl$myTPSlist) > 0))
pred = vector("list", k)


    for(i in 1:k) 
      if(is.null(cl$myTPSlist[[i]][[v]])) {
        pred[[i]] = NULL
      } else {
        pred[[i]] = mgcv::predict.bam(cl$myTPSlist[[i]][[v]], newdata = list(time = ntime),
                            type = "response")
      }
