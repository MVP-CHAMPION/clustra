...
data = gen_traj_data(vars, clusters = c(800, 1000,1200), m_obs = 25, 
                     s_range = c(-365, -90), e_range = c(0.5*365, 2*365),
                     min_obs = 10, cv = 0.00001, verbose = TRUE)
data$sys_std <- scale(data$sys)[, 1]
data$dia_std <- scale(data$dia)[, 1]
...
library(ggplot2)
library(gridExtra)
psys = ggplot(data, aes(x = time, y = sys, color = as.factor(true_group))) + 
  geom_point(data = data, aes(y = sys), pch = ".") #+ geom_line()


pdys = ggplot(data, aes(x = time, y = dia, color = as.factor(true_group))) + 
  geom_point(data = data, aes(y = dia), pch = ".")
