...
```{r}
library(clustra)
mc = 2 # If running on a unix or a Mac platform, increase up to 2x # cores
init = "distant" # maximize median absolute error for initial traj selection
set.seed(12345)
data = gen_traj_data(n_id = c(200, 400, 800, 1600), types = c(1, 2, 3, 2), 
                     intercepts = c(100, 80, 120, 130), m_obs = 25, 
...
sdat = data[id %in% sample(unique(data[, id]), 9)]
plot_sample(sdat)
```
...
```{r}
set.seed(1234737)
cl = clustra(data, k = 4, starts = init, maxdf = 10, conv = c(10, 0), mccores = mc,
             verbose = TRUE)
```
...
```{r fig.width = 7, fig.height = 9}
set.seed(1234567)
data2 = gen_traj_data(n_id = c(250, 500, 1000, 2000), types = c(1, 2, 3, 2), 
                     intercepts = c(100, 80, 120, 130), m_obs = 25,
                     s_range = c(-365, -14), e_range = c(60, 2*365), 
                     noise = c(0, 30))
plot_sample(data2[id %in% sample(unique(data2[, id]), 9)])

cl = clustra(data2, k = 4, starts = init, maxdf = 10, conv = c(10, 0),
...
```{r fig.width = 7}
set.seed(1234737)
sil = clustra_sil(data, kv = c(2, 3, 4), starts = init, mccores = mc, maxdf = 10,
...
```{r fig.width = 7, fig.height=7}
set.seed(1234737)
ran = clustra_rand(data, k = c(2, 3, 4), starts = init, mccores = mc, 
...
```{r fig.width = 7, fig.height = 7}
set.seed(12347)
cl = clustra(data2, k = 30, starts = init, maxdf = 10, conv = c(10, 0),
             mccores = mc, verbose = TRUE)
