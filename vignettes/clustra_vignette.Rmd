...
We also set *RNGkind* and *seed* for reproducibility. Code below generates the data, looks at a few observations of the generated data, and writes the data to a csv file in our working directory. The *mc* variable sets core use and will be assigned to *mccores* parameter through the rest of the vignette. By default, 1 core is assigned. Parallel sections are implemented with *parallel::mclappy()*, so on unix and Mac platforms it is recommended to use the full number of cores available for faster performance.
```{r}
library(clustra)
mc = 1
rng_prev = RNGkind("L'Ecuyer-CMRG")
set.seed(1234567)
data = gen_traj_data(n_id = c(500, 1000, 2000), m_obs = 25, 
...
```
The result is less perfect but still pretty good score. Now the plot:
```{r fig.width = 7, fig.height = 7}
sdt = data2
if(nrow(data) > 10000)
...
```{r fig.width = 7}
set.seed(1234737)
sil = clustra_sil(data, k = c(2, 3, 4), mccores = mc, verbose = TRUE)
plot_sil = function(x) {
...
rand_plot(ran)
```
The plot shows Adjusted Rand Index similarity level between all pairs of 30 clusterings (10 random starts for each of 2, 3, and 4 clusters). The ten random starts agree the most for *k*=3. From the deviance results shown during iterations, we also see that all of the *k*=3 clusters are near the best deviance attainable even with *k* = 4. Three of the *k* = 4 results also collapse to 3 clusters ("zerocluster" convergence note) and show the same cluster counts as the *k* = 3 cluster results. Further two agree on two cluster counts (1983 and 532) and split the third into two parts.
```{r}
## restore RNG
RNGkind(rng_prev[1])
