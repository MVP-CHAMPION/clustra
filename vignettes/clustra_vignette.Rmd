...
curvefun=function(paramlist){
  ...
  for(i in 1:length(paramlist)){
    clus1<-defDataAdd(varname="outv1",formula = paramlist[[i]][[2]],
                      variance=paramlist[[i]][[8]])
    clus1<-defDataAdd(clus1,varname = "outv2", formula = paramlist[[i]][[3]],
                      variance=paramlist[[i]][[8]])
    
    clus2<-defDataAdd(varname="outv1",formula = paramlist[[i]][[4]],
                      variance=paramlist[[i]][[8]])
    clus2<-defDataAdd(clus2,varname = "outv2", formula = paramlist[[i]][[5]],
                      variance=paramlist[[i]][[8]])
    
    clus3<-defDataAdd(varname="outv1",formula = paramlist[[i]][[6]],
                      variance=paramlist[[i]][[8]])
    clus3<-defDataAdd(clus3,varname = "outv2", formula = paramlist[[i]][[7]],
                      variance=paramlist[[i]][[8]])
...
datlist_f=curvefun(param_l)
  ...
  for(n in 1:numit){
    ...
    data[[n]] = gen_traj_data(vars, clusters = c(param_l[[scen]][[9]],param_l[[scen]][[9]],param_l[[scen]][[9]]), 
...
write_xlsx(df, paste0("C:/Users/Rachel/Desktop/Dissertation/Project 1 Simulation Output/",param_l[[scen]][[1]],"_runstart_",seedst,"_nrun",numit,".xlsx"))
```
