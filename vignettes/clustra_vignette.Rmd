...
clus1<-defDataAdd(varname="sys",formula="(1-(100/(1+2.71828^(-time+300))))+80",variance=1)

#clus1<-defDataAdd(varname="sys",formula="0.0375*time+85",variance=50)
clus1<-defDataAdd(clus1,varname="dia",formula="95",variance=50)

clus2<-defDataAdd(varname="sys",formula="65",variance=50)
clus2<-defDataAdd(clus2,varname="dia",formula="0.05*time+65",variance=50)

clus3<-defDataAdd(varname="sys",formula="-0.0375*time+45",variance=50)
clus3<-defDataAdd(clus3,varname="dia",formula="35",variance=50)

clus4<-defDataAdd(varname="sys",formula="0.0375*time+100",variance=2)
clus4<-defDataAdd(clus4,varname="dia",formula="60",variance=2)


clus5<-defDataAdd(varname="sys",formula="0.0375*time+65",variance=2)
clus5<-defDataAdd(clus5,varname="dia",formula="-0.1*time+45",variance=2)

clus6<-defDataAdd(varname="sys",formula="0.0375*time+20",variance=2)
clus6<-defDataAdd(clus6,varname="dia",formula="30",variance=2)

clus7<-defDataAdd(varname="sys",formula="0.0375*time+85", variance=10)
clus7<-defDataAdd(clus7,varname="dia",formula="40+sin(-100+3.14159*time/400)*10",variance=5)

clus8<-defDataAdd(varname="sys",formula="0.0375*time+90",variance=10)
clus8<-defDataAdd(clus8,varname="dia",formula="80+sin(-100+3.14159*time/300)*10",variance=2)

clus9<-defDataAdd(varname="sys",formula="75",variance=10)
clus9<-defDataAdd(clus9,varname="dia",formula="60",variance=5)

clus10<-defDataAdd(varname="sys",formula="75",variance=10)
clus10<-defDataAdd(clus10,varname="dia",formula="60",variance=5)
#####
#### changing pairing of first scenario
clus11<-defDataAdd(varname="sys",formula="0.0375*time+85",variance=2)
clus11<-defDataAdd(clus11,varname="dia",formula="95",variance=2)

clus12<-defDataAdd(varname="sys",formula="65",variance=2)
clus12<-defDataAdd(clus12,varname="dia",formula="35" ,variance=2)

clus13<-defDataAdd(varname="sys",formula="-0.0375*time+45",variance=2)
clus13<-defDataAdd(clus13,varname="dia",formula="0.05*time+65",variance=2)


clus14<-defDataAdd(varname="sys",formula="0.0375*time+85",variance=2)
clus14<-defDataAdd(clus14,varname="dia",formula="35" ,variance=2)

clus15<-defDataAdd(varname="sys",formula="65",variance=2)
clus15<-defDataAdd(clus15,varname="dia",formula="95" ,variance=2)

clus16<-defDataAdd(varname="sys",formula="-0.0375*time+45",variance=2)
clus16<-defDataAdd(clus16,varname="dia",formula="0.05*time+65",variance=2)
...
#curv3=c(14,15,16)
 ...
 curvlist=list(curv1)
  ...
  datlist=list(clus1,clus2,clus3,clus4,clus5,clus6,clus7,clus8,clus9,clus10, clus11,clus12,clus13,clus14,clus15,clus16)


  data = rep(list(NULL),length(curvlist))
  for(q in 1:length(curvlist)){
data[[q]] = gen_traj_data(vars, clusters = c(800, 1000,1200), m_obs = 10, 
                     curvlist=curvlist[[q]], 
                     s_range = c(-5, -90), e_range = c(0.5*365, 365),
...
library(gridExtra)
 ...
 for(s in 1:length(curvlist)){
psys = ggplot(data[[s]], aes(x = time, y = sys, color = as.factor(true_group))) + 
  geom_point(data = data[[s]], aes(y = sys), pch = ".") #+ geom_line()


pdys = ggplot(data[[s]], aes(x = time, y = dia, color = as.factor(true_group))) + 
  geom_point(data = data[[s]], aes(y = dia), pch = ".")
...
```{r}
cl = rep(list(NULL),length(curvlist))
 for(s in 1:length(curvlist)){
   set.seed(74847103)
...
for(s in 1:length(curvlist)){
 ## dat=data[[s]]
  data[[s]]=  data[[s]][, group:=factor(..cl[[s]]$data_group)]
}
```
