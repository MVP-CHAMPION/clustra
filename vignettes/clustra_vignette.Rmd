...
We also set a generator kind and seed for reproducibility, look at a few observations of the generated data, and write the data to a csv file.
```{r}
rng_prev = RNGkind("L'Ecuyer-CMRG")
set.seed(1234567)
data = gen_traj_data(n_id = 3000, m_obs = 25, s_range = c(-50, -10),
                     e_range = c(3*365, 10*365), noise = c(0, 5))
head(data)
...
```{r}
set.seed(1234737)
cl = clustra(data, k = 3, fp = list(maxdf = 30, iter = 10, starts = 4, idperstart = 20, retry_max = 3), cores = c(e_mc = 1, m_mc = 1, nthreads = 1, blas = 1), verbose = TRUE)
```
...
for(i in 1:k) 
  if(is.null(cl$tps[[i]])) {
    ...
    pred[[i]] = predict(cl$tps[[i]], newdata = list(time = ntime),
                        type = "response")
  }
...
```{r fig.width = 7, fig.height = 9}
set.seed(1234567)
data2 = gen_traj_data(n_id = 3000, m_obs = 25, s_range = c(-50, -10),
                     e_range = c(3*365, 10*365), noise = c(0, 20))
...
cl = clustra(data2, k = 3, fp = list(maxdf = 30, iter = 10, starts = 4, idperstart = 20, retry_max = 3), cores = c(e_mc = 1, m_mc = 1, nthreads = 1, blas = 1), verbose = TRUE)
MixSim::RandIndex(cl$data_group, data2[, true_group])
```
Next, we request too many clusters.
```{r}
set.seed(1234737)
cl = clustra(data2, k = 20, fp = list(maxdf = 30, iter = 20, starts = 4, idperstart = 20, retry_max = 3), cores = c(e_mc = 1, m_mc = 1, nthreads = 1, blas = 1), verbose = TRUE)
```
Next, plot the resulting fit with a sample of data, colored by the cluster value.
```{r fig.width = 7, fig.height = 7}
...
rand_plot(ran)
```
The plot shows similarity level between all pairs of 30 clusterings (10 random starts for each of 2, 3, and 4 clusters). The ten random starts agree the most for *k*=3, which also approach the smallest deviance values among all starts and *k* values. But there is also a separate local deviance minimum for two of the 3-cluster starts. Also, interestingly, one of the four cluster attempts collapses to the optimal 3 clusters. In addition, Five of the two-cluster starts are consistent with the 3-cluster optimum.  
```{r}
## restore RNG
RNGkind(rng_prev[1])
```
