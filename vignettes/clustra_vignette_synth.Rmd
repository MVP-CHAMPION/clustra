...
This particular vignette is aimed to demonstrate the **clustra** package features to a synthetic dataset of systolic blood pressure based on real US Veteran data. We simulated 80,000 individuals based on five clusters that were identified in a sample of predominately male US Veterans initiating anti-hypertensive medication (set as day 0). Individuals were restricted based on having at least 2 primary care visits in the years preceding drug initiation, with a least one SBP measurement in the year prior to treatment initiation, and a minimum of four SBP observations through the two years following treatment initiation. 

The example, below, demonstrates the use of the R package commands to identify the correct number of clusters from this data using a linux-based machine with a set number of cores. We demonstrate that the clusters identified using the R package and SAS macros produce nearly identical results, and how to obtain the corresponding components from each computing platform.

We start by loading the necessary R packages and grabbing the data that was generated for this example.
```{r, setup.libs, warning=FALSE}
library(clustra)
start_knit = deltime()
...
```{r, getdata}
mc = 5 
# If running on a unix or a Mac platform, change to number of cores
# mc = detectCores()
print(paste("Number of cores being used: mc parameter =", mc))
data = read.csv('~/Git/mvp-champion/sas-trajectories-macros/Simulated Data/simulated_data_27June2023.csv')
data <- data.table(data)
setnames(data, 'group', 'true_group')
head(data)
# input the ouput path for the plots
plot_path <- "~/Git/go-ski/clustra/vignettes/"
```
Select a few random *id*s and print their scatterplots. Note that this only displays 9 individuals, and the data was generated based on five clusters. As the data used to simulate this synthetic dataset is extremely noisy, we expect that it would be difficult to ascertain any patterns by looking at so few subjects.
```{r, plot_true, fig.width = 7, fig.height = 9}
set.seed(12345)
plot_sample(data, layout = c(3, 3), group = "true_group")
```
This is a plot a 20,000 sample of the raw data. Use the true or simulated group assignment for coloring. As we can see, there is a lot of noise and it would be difficult to identify the clusters of trajectories by looking at the individual points, alone.
```{r, plotraw, fig.width = 7, fig.height = 7}
plot_smooths(data, max.data = 20000, group = "true_group")
```

Next, cluster the trajectories. Set *k=5*, spline max degrees of freedom to 30, and maximum iterations to 20. *mccores* sets the number of cores to use in various components of the code. This particular example uses $mc$ cores on a linux-based machine. We print out the full information for the iterations, as well as, the runtime in hours.
```{r, run5}
set.seed(12345)
t = deltime()

cl = clustra(data, k = 5, maxdf = 30, conv = c(20, 0.5), mccores = mc, verbose = TRUE)
t = deltime(t, paste("seconds of run time on", mc, "cores"))

# Look at what happens if we cut the iterations at 5 maximum
set.seed(12345)
cl.v2 = clustra(data, k = 5, maxdf = 30, conv = c(5, 0.5), mccores = mc, verbose = FALSE)
```

Iteration of plots - We can look at the individual clustering algorithm progression by setting `verbose = 2`.
...
Plot the final 5 cluster results.
```{r, plot5, fig.width = 7, fig.height = 5, warnings=FALSE}
...
set.seed(12345)
start2 = deltime()
cl10 = clustra(data, k = 10, maxdf = 30, conv = c(50, 0.5), mccores = mc, verbose = FALSE)
deltime(start2, paste("run time on", mc, "cores"))
...
```{r, echo=FALSE}
#Save the results to the data
data$clus10 <- cl10$data_group
```

Next, we look at what the clusters would look like with 2 groups.
```{r, run2, fig.width = 7, fig.height = 5}
set.seed(12345)
t = deltime()
cl2 = clustra(data, k = 2, maxdf = 30, conv = c(20,0.5), mccores=mc, verbose = FALSE)
deltime(t, paste("Seconds run time on", mc, "cores")) # (1.2 minutes on 8 cores)
...
pdf(paste0(plot_path, "R_cl2_plot.pdf"))
  plot_smooths(data, cl2$tps, max.data = 0)
dev.off()
knitr::include_graphics("R_cl2_plot.pdf")

MixSim::RandIndex(cl2$data_group, data[, true_group])$AR

#Save the results to the data
data$clus2 <- cl2$data_group
```

For the resulting cluster results, what does the Adjusted Rand Index look like?
```{r, comp.within}
...
t = deltime(t, paste("Seconds silhouettes on", mc, "cores"))
...
sil = clustra_sil(data, k = c(3,4,20), mccores = mc, conv = c(50,0.5), 
                  verbose = FALSE)
lapply(sil, plot_silhouette)
t = deltime(t, paste("Seconds silhouettes and clustra on", mc, "cores"))
```
...
```{r, randplot, fig.width = 7, fig.height=7}
set.seed(12345)
t = deltime()
ran = clustra_rand(data, k = seq(2, 10, 1), starts = "random", mccores = mc, 
                   replicates = 10, conv=c(40,0.5), verbose = TRUE, save = TRUE)
t = deltime(t, paste("Seconds clustra_rand on", mc, "cores")) # 4c-i7 mac 5 cores  seconds
rand_plot(ran)
ggsave(filename = "R_randplot.png", path=plot_path)
```
...
```{r, hclust, fig.width = 7, fig.height = 5}
set.seed(12345)
t = deltime()
cl40 = clustra(data, k = 40, maxdf = 30, conv = c(100,0.5), mccores = mc, verbose = FALSE)
...
deltime(t, paste("Seconds clustra k = 40, conv = c(100, 0.5) on", mc, "cores")) # runtime in minutes
```
...
```{r echo=FALSE}
deltime(start_knit, "clustra vignette run time in seconds")
```
